...
class BrowserSession:
    ...
    async def __aenter__(self):
        ...
        self._browser = await self._playwright.chromium.launch(
            ...
            args=self._browser_config.get('args', ['--disable-blink-features=AutomationControlled'])
        )
        self._page = await self._browser.new_page()
        await self._page.set_extra_http_headers(self._browser_config.get('headers', self._DEFAULT_HEADERS))
        await self._page.add_init_script(
            self._browser_config.get('init_script', "Object.defineProperty(navigator, 'webdriver', {get: () => false})")
        )
        return self
