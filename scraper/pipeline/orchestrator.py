...
class Orchestrator:
    ...
    def handle_error(self, pair: TrackingPair, error: Union[str, Exception]) -> AvailabilityResult:
        """
    ...
    async def _run_pair_path_a(self, pair: TrackingPair, session: BrowserSession) -> dict:
        """
        Path A: Discovery.
        ...
        if store['search_url_template'] is None and crawl_result.get('search_url_template'):
            self.db_writer.update_store_search_template(pair['store_id'], crawl_result['search_url_template'])
    ...
    async def _run_pair_path_b(self, pair: TrackingPair, session: BrowserSession) -> dict:
    ...
    async def _run_pair_path_c(self, pair: TrackingPair, session: BrowserSession) -> dict:
    ...
    async def _run_pair_path_d(self, pair: TrackingPair, session: BrowserSession) -> dict:
